...
public class Tab3PostFragment extends BaseTab3Fragment<Post> implements RecycleViewPostAdapter.OnPostActionListener {
    ...
    protected List<Post> getData() {
        ...
        List<Post> postReviewedList = new ArrayList<>();

        if (postList == null || postList.isEmpty()) {
            showToast("No posts available.");
            return postReviewedList; // Return empty list if no posts are available
        }

        for (Post post : postList) {
            VerPost verPost = verPostDao.getByPostId(post.getPostId());
            if (verPost != null && !"pending".equals(verPost.getVerifiedStatus())) {
                postReviewedList.add(post);
            }
        }

        if (postReviewedList.isEmpty()) {
            showToast("No reviewed posts available.");
        }

        return postReviewedList;
    }
